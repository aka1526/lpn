                        ...
                        <div class="card-header tx-medium bd-0 text-white bg-primary ">
                            <div class="d-flex justify-content-between">
                                <h4 class="card-title mg-b-0 text-white">
                                    <i class="fas fa-book-medical"></i> Khan Table
                                </h4>
                                <div class="d-flex my-xl-auto right-content">
                                    <div class="pr-1  mb-xl-0">

                                        <a class="btn btn-indigo  btn-block "  
                                         href="{{ route('khans.new')}}"><i class="fas fa-book-medical"></i>
                                            New Khan</a>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered mg-b-0 text-md-nowrap">
                                    <thead>
                                        ...
                                        <tr class="bg-info ">
                                            <th class="text-center">Khan No</th>
                                            <th>Group</th>
                                            <th>Khan Name</th>
                                            <th>Description</th>
                                    ...
                                    <tbody>
                                        @foreach ($khans as $key => $row)
                                            <tr>
                                                <th class="text-center" scope="row"> {{ $khans->firstItem() + $key }}
                                                </th>
                                                <td>{{ $row->khan_group }}</td>
                                                <td>{{ $row->khan_name }}</td>
                                                <td>{!! $row->khan_desc !!}</td>
                                                ...
                                                <span class="badge
                                                    {{ $row->khan_status=='Y' ? 'badge-primary' :'badge-secondary' }} ">
                                                    {{ $row->khan_status=='Y' ? 'Enable' :'Disable' }}</span>  
                                                    </h5>
                                                </td>
                                                <td class="text-center">
                                                    <div class="btn-icon-list">
                                                        ...
                                                        <button
                                                            ...
                                                            {{ $row->khan_status == 'Y' ? 'bg-secondary' : 'bg-success' }} "
                                                            data-uid="{{ $row->khan_uid  }}"
                                                            data-status="{{ $row->khan_status == 'Y' ? 'N' : 'Y' }}"
                                                            data-toggle="tooltip" title="Edit Status"> 
                                                            <i class="{{ $row->khan_status == 'Y' ? 'fas fa-eye-slash' : 'fas fa-eye' }}"></i> 
                                                            </button>
                                                                
                                                        <a href="/pageadmin/khans/edit/{{ $row->khan_uid }}" class="btn btn-indigo btn-icon btn-sm  "
                                                           
                                                            title="Edit Course"> <i class="fa fa-edit"></i></a>

                                                        <button class="btn btn-danger btn-icon btn-sm btn-delete"
                                                            data-uid="{{ $row->khan_uid  }}" data-toggle="tooltip"
                                ...
                                <div class="pt-2">
                                    {{ $khans->links('pagination.default', ['paginator' => $khans, 'link_limit' => $khans->perPage()]) }}
                                </div>
                            </div>
                        </div>
                    </div>
