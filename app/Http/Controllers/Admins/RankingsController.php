...
class RankingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // if ($action) {
        //     Rankingslist::where('list_ref', '=', $uid)->update([
        //         "contenders_gander" => $request->rank_gander,
        //     ]);
        // }

        if ($imagename != '') {
            Rankings::where('list_ref', '=', $uid)->update([
                "rank_img" => $imagename,
            ]);
    ...
    function list(Request $request, $uid) {
        if ($this->GetUserUid() == '') {
            return redirect(url('/pageadmin/adminlogin'));
        }
    ...
    public function add_list(Request $request)
    {
        ...
        if ($rankings) {
            ...
            $action = Rankingslist::insert([
                ...
                , "contenders_type" => $request->contenders_type
                , "contenders_gander" => $request->contenders_gander
                , "won_title" => $request->won_title
                , "last_defense" => $request->last_defense
                , 'created_by' => $created_by
            ...
            if ($request->contenders != '') {
                if ($request->contenders_type == 'WORLD') {

                    Rankings::where('rank_uid', '=', $rank_uid)->update([
                        'world_vacant' => $request->contenders
                        , "world_won_title" => $request->won_title
                        , "world_last_defense" => $request->last_defense
                        , "rank_img" => $imagename
                        , "world_img" => $imagename,
                ...
                if ($request->contenders_type == 'INTERNATIONAL') {

                    Rankings::where('rank_uid', '=', $rank_uid)->update([
                        'international_vacant' => $request->contenders
                        , "international_won_title" => $request->won_title
                        , "international_last_defense" => $request->last_defense
                        , "rank_img" => $imagename
                        , "international_img" => $imagename,
                    ]);
                }
            ...
            if ($imagename != '') {
                ...
                Rankingslist::where('list_uid', '=', $uid)->update([
                    "contenders_img" => $imagename,
                ]);
